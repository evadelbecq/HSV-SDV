...
interface AppointmentForm {
  ...
  specialite: string;
  nomPraticien: string; // On va stocker le doctor_id ici au lieu du nom
...
const AppointmentBooking: React.FC = () => {
  const [formData, setFormData] = useState<AppointmentForm>({
    ...
    heureRendezVous: '',
    specialite: '',
    nomPraticien: '', // Stockera le doctor_id en string
    motifConsultation: ''
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        if (doctorsResponse && doctorsResponse) {
          setDoctors(doctorsResponse);
          console.log('Docteurs chargés:', doctorsResponse);
        } else {
          console.warn('Aucune donnée de docteur reçue');
          setDoctors([]);
        }
        
        // Charger les spécialisations avec leurs raisons
        const specializationsResponse = await api.getSpecializations();
        console.log('Réponse spécialisations:', specializationsResponse);
        
        if (specializationsResponse) {
          setSpecializations(specializationsResponse);
          console.log('Spécialisations chargées:', specializationsResponse);
        } else {
          console.warn('Aucune donnée de spécialisation reçue');
          setSpecializations([]);
        }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Maintenant on a directement le doctor_id
      ...
      // Le backend retourne directement l'objet appointment créé en cas de succès (status 201)
      ...
      if (error.response) {
        // Le backend retourne { message: 'Internal server error' } en cas d'erreur 500
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 border-b-2 border-gray-300 focus:border-green-500 outline-none transition-colors bg-transparent text-gray-500"
                  required
                  disabled={isLoading}
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 border-b-2 border-gray-300 focus:border-green-500 outline-none transition-colors bg-transparent appearance-none cursor-pointer"
                    required
                    disabled={isLoading}
                  >
                ...
                <small className="text-gray-500 mt-1 block">
                  {specializations.length} spécialité(s) chargée(s)
                </small>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    onChange={handleInputChange}
                    className="w-full px-4 py-3 border-b-2 border-gray-300 focus:border-green-500 outline-none transition-colors bg-transparent appearance-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
                    required
            ...
            <div className="flex justify-center mb-8">
              <div className="flex items-center space-x-3 p-4 border border-gray-300 rounded bg-gray-50">
                <input
                  ...
                  disabled={isLoading}
                />
                <label htmlFor="robot-check" className="text-sm text-gray-700">
                  I'm not a robot
                </label>
                <div className="ml-4">
            ...
            <div className="flex justify-center">
              <button
                ...
                disabled={isLoading}
              >
                {isLoading ? 'Création en cours...' : 'Confirmer mon rendez-vous'}
              </button>
            </div>
          </form>
        </div>
